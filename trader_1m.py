#!/usr/bin/env python3
"""V4 1分钟自适应交易 — PAPER_MODE(纸盘) + 实盘双模式"""
import subprocess, json, time, hmac, base64, hashlib, os
from datetime import datetime, timezone
import pandas as pd, numpy as np

# ===== 模式切换 =====
# PAPER_MODE=True:  真实K线，虚拟持仓，不下单到OKX（纸盘模拟）
# PAPER_MODE=False: 真实下单到OKX实盘
PAPER_MODE = os.environ.get("PAPER_MODE", "true").lower() in ("true", "1", "yes")
...
def okx_request(method, path, body=""):
    ...
    if not r.stdout.strip():
        return {'code': '-1', 'msg': 'OKX API unreachable', 'data': []}
    try:
        return json.loads(r.stdout)
    except json.JSONDecodeError:
        return {'code': '-1', 'msg': 'Invalid JSON', 'data': []}
...
def load_paper_state():
    try:
        ...
        return {"balance": 1727.0, "position": None, "trades": [],
                "start_balance": 1727.0, "start_time": datetime.now().isoformat()}
...
def get_positions():
    if PAPER_MODE:
        ...
        if pos:
            return [{"posSide": pos["side"], "pos": str(pos["sz"]),
                     "avgPx": str(pos["entry"]),
                     "upl": str(_paper_upl(pos))}]
        return []
...
def _paper_upl(pos):
    """Compute paper unrealized PnL"""
    df = fetch_1m_bars(2)
    if df is None or len(df) == 0:
        return 0
    curr = df['c'].iloc[-1]
    if pos["side"] == "long":
        return pos["sz"] * 0.01 * (curr - pos["entry"])
    else:
        return pos["sz"] * 0.01 * (pos["entry"] - curr)

def place_order(side, sz, pos_side=None):
    if PAPER_MODE:
        df = fetch_1m_bars(2)
        price = df['c'].iloc[-1] if df is not None and len(df)>0 else 0
        fee = price * 0.01 * sz * 0.001  # 0.1% fee
        PAPER["balance"] -= fee
        PAPER["position"] = {"side": pos_side or ("long" if side=="buy" else "short"),
                             "sz": sz, "entry": price, "ts": 0}
        PAPER["trades"].append({
            "time": datetime.now().isoformat(),
            "action": "open", "side": side, "sz": sz, "price": price, "fee": fee
        })
        save_paper_state(PAPER)
        return {"code": "0", "msg": "", "data": [{"ordId": f"paper_{len(PAPER['trades'])}"}]}

    order = {'instId': INST_ID, 'tdMode': 'cross', 'side': side,
             'ordType': 'market', 'sz': str(sz)}
    if pos_side:
        order['posSide'] = pos_side
    return okx_request('POST', '/api/v5/trade/order', json.dumps(order))
...
def close_position(side, sz):
    ...
    if PAPER_MODE:
        ...
        df = fetch_1m_bars(2)
        price = df['c'].iloc[-1] if df is not None and len(df)>0 else pos["entry"]
        fee = price * 0.01 * sz * 0.001
        if pos["side"] == "long":
            pnl = sz * 0.01 * (price - pos["entry"])
        else:
            pnl = sz * 0.01 * (pos["entry"] - price)
        net = pnl - pos["entry"] * 0.01 * sz * 0.001  # opening fee too
        PAPER["balance"] += net - fee + pos["entry"] * 0.01 * sz * 0.001  # return capital
        PAPER["balance"] = max(0, PAPER["balance"])  # no negative
        PAPER["trades"].append({
            "time": datetime.now().isoformat(),
            "action": "close", "side": side, "sz": sz,
            "price": price, "pnl": pnl, "net": net, "fee": fee
        })
        PAPER["position"] = None
        save_paper_state(PAPER)
        return {"code": "0", "msg": "", "data": [{"ordId": f"paper_{len(PAPER['trades'])}"}]}
...
# ===== 主程序 =====
def main():
    mode_label = "🧪 纸盘模拟" if PAPER_MODE else "💰 实盘交易"
    print(f"═══ V4 1m自适应 [{mode_label}] ═══")
    ...
    if signal and positions:
        ...
        if r.get('code') == '0':
            ...
            print(f"   ❌ {r.get('msg', str(r)[:100])}")

    if signal and not positions and balance < 0:
        print(f"🔔 信号: {signal['direction']} @ {signal['price']:.1f} | 无法下单")

    # 纸盘统计
    if PAPER_MODE and PAPER["trades"]:
        closed = [t for t in PAPER["trades"] if t["action"] == "close"]
        if closed:
            wins = [t for t in closed if t["net"] > 0]
            total_pnl = sum(t["net"] for t in closed)
            print(f"📈 纸盘: {len(closed)}笔 | 胜{len(wins)}/{len(closed)-len(wins)} "
                  f"| 累计{total_pnl:+.2f}U | 余额{PAPER['balance']:.2f}U")
